pipeline {
  environment {
    ARGO_SERVER = '35.185.64.11:32100'
    DEV_URL = 'http://35.185.64.11:30080/'
  }
  agent {
    kubernetes {
        ...
        stage('Image Linting') {
          steps {
            container('docker-tools') {
              sh 'dockle docker.io/jerry871002/dso-demo'
            }
          }
        }
        stage('Image Scan') {
          steps {
            container('docker-tools') {
              sh 'trivy image --timeout 10m --exit-code 1 jerry871002/dso-demo'
            }
          }
        }
      }
